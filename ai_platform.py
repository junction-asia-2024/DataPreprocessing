...
async def analyze_data_in_batches(df: pd.DataFrame, batch_size: int = 5) -> list:
    # 데이터를 배치로 나누어 처리
...
def build_summary(analyses: list) -> str:
    # 각 배치 분석 결과를 요약하고, 종합한 최종 요약 생성
    combined_text = "\n".join(analyses)
    summary_prompt = f"""
    다음은 각 배치의 분석 결과를 요약한 내용입니다:
    {combined_text}

    위 내용을 바탕으로 전체 분석 결과에 대한 최종 요약을 제공해 주세요: 
    1. 전체 분석 결과 요약
    2. 주요 패턴 및 트렌드 요약
    3. 지역별 도로 손상 정보 요약
